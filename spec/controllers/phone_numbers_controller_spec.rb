...
RSpec.describe PhoneNumbersController, type: :controller do
  let(:jen) { Person.create(first_name: 'Jen', last_name: 'Hill') }
  let(:valid_attributes) {
    {number: "MyString", person_id: 1}
  }

  let(:harry) { Person.create(first_name: 'Harry', last_name: 'Jones')}
  ...
  describe "POST #create" do
    context "with valid params" do
       let(:alice) { Person.create(first_name: 'Alice', last_name: 'Smith') }
        let(:valid_attributes) { {number: '303-555-1234', person_id: alice.id} }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:bob) { Person.create(first_name: 'Bob', last_name: 'Jones') }
        let(:valid_attributes) { {number: 'MyNewString', person_id: bob.id } }
